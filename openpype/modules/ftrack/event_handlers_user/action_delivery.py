...
from openpype.lib.ttd_op_utils import handle_csv, augment_repre_with_ftrack_version_data
...
class Delivery(BaseAction):
    ...
    def launch(self, session, entities, event):
        ...
        try:
            ...
            self.log.warning("Error during processing delivery action.", exc_info=True)
    ...
    def generate_version_by_repre_id_dict(self, session: Session, entities: List[Entity]):
        """Create a dict with repres ids as keys and Ftrack AssetVersions as values.
        
        This is used later for quickly fetching the AssetVersion from a OP repre
        and updates attributes on it.
        """
    ...
    def real_launch(self, session, entities, event):
        ...
        for repre in repres:
            ...
            if template_override and repre_report_items and original_template:
                ...
                anatomy.templates_obj.set_templates(raw_anatomy_templates)
                format_dict = get_format_dict(anatomy, location_path)
                new_repre_report_items = check_destination_path(
                    repre["_id"],
                    anatomy,
                    anatomy_data,
                    datetime_data,
            ...
            augment_repre_with_ftrack_version_data(repre, version, ftrack_list_name)
